...
function loadrepos (xhr) {
  var ojson = xhr;
  if (xhr.items !== undefined) { ojson = xhr.items; } else { ojson = xhr; }
  var s = '';
...
function loadcontents (data, reponame, owner, path) {
  ...
  for (var i = 0, j = ojson.length; i < j; i++) {
    o = ojson[i];
    if ((o.type == 'file') && (o.name.indexOf('.xml') > 0)) { s = s + '<p class="pure-button transform" file="' + o.download_url + '"><i class="fa fa-angle-double-right"></i>' + o.name + '</p>'; } else if (o.type == 'dir') {
...
function init () {
  ...
  cdabody.find('div.section').each(function () {
    ...
    $(this).find('table').each(function () {
      ...
      if (c > 0) {
        if (c == 1) { var s = $('<tr class="all" style="cursor:pointer"><td colspan="5"><i class="fa fa-warning"></i> (' + c + ') duplicate row hidden. Click here to <span class="show">show</span>.</td></tr>'); } else { var s = $('<tr class="all" style="cursor:pointer"><td colspan="5"><i class="fa fa-warning"></i> (' + c + ') duplicate rows hidden. Click here to <span class="show">show</span>.</td></tr>'); }
      ...
      if (c > 0) {
        if (c == 1) { var s = $('<tr class="first" style="cursor:pointer"><td colspan="5"><i class="fa fa-question-circle"></i> (' + c + ') potential duplicate row. Click here to <span class="show1">hide</span>.</td></tr>'); } else { var s = $('<tr class="first" style="cursor:pointer"><td colspan="5"><i class="fa fa-question-circle"></i> (' + c + ') potential duplicate row. Click here to <span class="show1">hide</span>.</td></tr>'); }
  ...
  $('.toc').off('click').click(function () {
    ...
    if ($('li.hide.toc[data-code]').length != 0) {
      if (th.find('i.fa-warning').length == 0) { th.prepend('<i class="fa fa-warning fa-lg" style="margin-right:0.5em" title="Sections are hidden"></i>'); }
    } else {
...
function adjustWidth (section) {
  ...
  if (section.find('table').length > 0) {
    if (section.find('table').width() > section.width()) { section.width(section.find('table').width() + 20); }
  }
  $('#cdabody').packery();
}
...
function comparer (index) {
  return function (a, b) {
    var valA = getCellValue(a, index); var valB = getCellValue(b, index);
